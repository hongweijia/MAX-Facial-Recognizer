...
label_prediction = api.model('LabelPrediction', {
    'detection_box': fields.List(fields.Float(required=True, description='Bounding box for the detected face')),
    'probability': fields.Float(required=True, description='Probability of the detected face'),
    'embedding': fields.List(fields.Float(required=True, description='Embedding for the detected face'))
})
predict_response = api.model('ModelPredictResponse', {
    'status': fields.String(required=True, description='Response status message'),
    'predictions': fields.List(fields.Nested(label_prediction), description='Bounding boxes, probabilities, and '
...
input_parser.add_argument('image', type=FileStorage, location='files', required=True, help='An image file encoded as PNG, '
                                                                                          'Tiff or JPEG with an '
                                                                                          'arbitrary size')
...
class Predict(Resource):
    ...
    def post(self):
        ...
        label_preds = [{'detection_box': p[0][0:4], 'probability': p[0][4], 'embedding': p[1].tolist()} for p in preds]
