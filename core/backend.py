...
class ModelWrapper(object):
    ...
    def warmup(self):
        sample = np.random.randint(0, 255, [1, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, 3], dtype=np.uint8)
        self.gen_embedding(sample)


    def detect_faces(self, image):
        bboxes, faces = run_mtcnn(image, self.pnet, self.rnet, self.onet, image_size=DEFAULT_IMAGE_SIZE)
        return bboxes, faces


    def create_input_pipeline(self, input_queue, image_size, nrof_preprocess_threads, batch_size_placeholder):
    ...
    def load_embedding_model_help(self, model_path, nrof_preprocess_threads=1):
        image_placeholder = tf.placeholder(tf.uint8, (None, DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, 3),
                                           name="input_images")
        ...
        eval_input_queue = data_flow_ops.FIFOQueue(capacity=2000000,
                                                   ...
                                                   shapes=[(DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, 3), (1,), (1,)],
    ...
    def gen_embedding(self, faces):
        ...
        for _ in range(num_batches):
            ...
            embs, labels = self.sess_embedding.run([self.embeddings, self.label_batch], feed_dict=feed_dict)
    ...
    def predict(self, x):
        ...
        if(len(faces) > 0):
            ...
            results = [(bboxes[label], emb) for (label, emb) in zip(all_labels, all_embeddings)]
            return results
        else:
